...
class SerializersTestCase(TestCase):
    ...
    def test_movie_serializer_with_ombdapi_data_remote(self):
        ...
        remote_data = requests.get(
            f"http://www.omdbapi.com/?t=django&type=movie&apikey={api_key}"
        )
        old_count = Movie.objects.count()
...
class MovieRemoteRequestsTestCase(TestCase):
    ...
    def test_client_get_request_movie_one_or_multiple(self):
        ...
        self.assertEqual(
            response_serialized.data[0]["Title"], self.movie_1_data["Title"]
        )
        ...
        self.assertEqual(
            response_serialized.data[1]["Title"], self.movie_2_data["Title"]
        )
        with self.assertRaises(IndexError):
...
class ReportsConfigTest(TestCase):
    def test_apps(self):
        ...
        self.assertEqual(apps.get_app_config("movie").name, "movie")
